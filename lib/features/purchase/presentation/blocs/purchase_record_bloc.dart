...
class PurchaseRecordBloc
    extends Bloc<PurchaseRecordEvent, PurchaseRecordState> {
  PurchaseRecordBloc() : super(PurchaseRecordInitial()) {
    on<LoadPurchaseRecordEvent>((event, emit) async {
      emit(PurchaseRecordLoading());
      try {
        final data = await PurchaseRecordRepository.fetchData(event.fromDate, event.toDate);
